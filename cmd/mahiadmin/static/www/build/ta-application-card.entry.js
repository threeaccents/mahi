...
const TaApplicationCard = class {
    ...
    render() {
        ...
        return (h(Host, null, h("stencil-route-link", { url: `/applications/${this.application.slug}` }, h("div", { class: "card-wrapper" }, h("div", { class: "card-top" }, h("div", { class: "icons" }, h("ta-icon", { icon: "folder" }), this.me.isAdmin ?
            h("ta-dropdown", { onTaOverlayRendered: this.handleOverlayRendered, ref: (el) => this.dropdownEl = el, overlay: this.getEditMenu() }, h("div", { class: {
                    "vertical-dots-icon": true,
                    "active": this.isMenuShown,
                } }, h("ta-icon", { icon: "vertical-dots" }))) : null), h("div", { class: "avatars" }, h("ta-avatar-list", { size: "small", items: this.application.users.map(u => u.firstName) }))), h("div", { class: "card-bottom" }, h("div", { class: "card-title" }, this.name()), h("div", { class: "card-subtitle" }, this.application.storageEngine, " - ", formatBytes(this.application.storage))))), this.confirmDeleteModal));
    }
};
