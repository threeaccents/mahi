...
const TaApplicationListCard = class {
    ...
    render() {
        ...
        return (h(Host, null, h("stencil-route-link", { url: `/applications/${this.application.slug}` }, h("div", { class: "wrapper" }, h("div", { class: "left-side" }, h("div", { class: "icon" }, h("ta-icon", { icon: "folder" })), h("div", { class: "name" }, this.name()), h("div", { class: "storage" }, this.application.storageEngine, " - ", formatBytes(this.application.storage))), h("div", { class: "right-side" }, h("div", { class: "avatars" }, h("ta-avatar-list", { size: "small", items: this.application.users.map(u => u.firstName) })), this.me.isAdmin ?
            h("ta-dropdown", { onTaOverlayRendered: this.handleOverlayRendered, ref: (el) => this.dropdownEl = el, overlay: this.getEditMenu() }, h("div", { class: {
                ...
                } }, h("ta-icon", { icon: "vertical-dots" }))) : null))), this.confirmDeleteModal));
    }
};
